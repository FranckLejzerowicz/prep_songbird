...
from prep_songbird._io_utils import get_prjct_anlss_nm, get_job_folder


class CreateScripts(object):

    def __init__(self, config, prjct_nm, run_params, slurm):
        self.config = config
    ...
    def run_xpbs(self, sh, pbs_slm, nlss, params, dat,
        ...
        job = '%s.%s.%s' % (self.prjct_nm, nlss, dat)
        if idx != 'None':
            job = '%s_%s' % (job, idx)
        if self.config.jobs:
